			...
			foreach( $rows as $row ) : ?>
				<div class="happyforms-table__row happyforms-table__row--body" id="<?php echo esc_attr( $row['id'] ); ?>">
					...
					<?php foreach( $columns as $c => $column ) : ?>
						...
						?>
						<div class="happyforms-table__cell<?php echo $column_classes; ?>" style="width: <?php echo 100 / $columns_num; ?>%">
							...
							<?php if ( ! $part['allow_multiple_selection'] ) : ?>
								...
								<input type="radio" class="happyforms-visuallyhidden" name="<?php happyforms_the_part_name( $part, $form ); ?>[<?php echo esc_attr( $row['id'] ); ?>]" value="<?php echo $c; ?>" <?php echo $checked; ?> <?php happyforms_the_part_attributes( $part, $form ); ?> <?php echo ( empty( $column['submissions_left'] ) && '' != $column['limit_submissions_amount'] ) ? 'disabled' : '' ?>>
							...
							<?php else: ?>
								...
								?>
								<input type="checkbox" class="happyforms-visuallyhidden" name="<?php happyforms_the_part_name( $part, $form ); ?>[<?php echo esc_attr( $row['id'] ); ?>][]" value="<?php echo $c; ?>" <?php echo $checked; ?> <?php happyforms_the_part_attributes( $part, $form ); ?> <?php echo ( empty( $column['submissions_left'] ) && '' != $column['limit_submissions_amount'] ) ? 'disabled' : '' ?>>

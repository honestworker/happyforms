...
class HappyForms_Part_Rank_Order extends HappyForms_Form_Part {
	...
	public function customize_templates() {
		$template_path = happyforms_get_include_folder() . '/templates/parts/customize-rank-order.php';
	...
	public function frontend_template( $part_data = array(), $form_data = array() ) {
		...
		$template_path = happyforms_get_include_folder() . '/templates/parts/frontend-rank-order.php';
	...
	public function style_dependencies( $deps, $forms ) {
		...
		if ( ! happyforms_is_preview() && ! $contains_rank_order ) {
			return $deps;
		}

		wp_register_style(
			'happyforms-rank-order',
			happyforms_get_plugin_url() . 'inc/assets/css/frontend/rank-order.css',
			array(), happyforms_get_version()
		);

		$deps[] = 'happyforms-rank-order';
	...
	public function customize_enqueue_scripts( $deps = array() ) {
		wp_enqueue_script(
			'part-rank-order',
			happyforms_get_plugin_url() . 'inc/assets/js/parts/part-rank-order.js',
	...
	public function validate_value( $value, $part = array(), $form = array() ) {
		if ( 1 === $part['required'] ) {
			...
			if( $has_empty_field ) {
					$error = new WP_Error( 'error', happyforms_get_validation_message( 'no_selection' ) );
					return $error;
			}
		}
	...
	public function script_dependencies( $deps, $forms ) {
		...
		if ( ! happyforms_is_preview() && ! $contains_rank ) {
			return $deps;
		}

		wp_register_script(
			'happyforms-part-rank-order',
			happyforms_get_plugin_url() . 'inc/assets/js/frontend/rank-order.js',
			array(), happyforms_get_version(), true
		);

		$deps[] = 'happyforms-part-rank-order';

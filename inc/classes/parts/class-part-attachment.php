...
class HappyForms_Part_Attachment extends HappyForms_Form_Part {
	...
	public function __construct() {
		...
		add_filter( 'happyforms_get_form_data', array( $this, 'transition_allowed_file_extensions' ), 99 );
	}
		...
		$fields = array(
			...
			'allowed_file_extensions' => array(
				...
				'sanitize' => array(
					...
					happyforms_allowed_file_extensions(),
				),
			),
			'max_file_size' => array(
				'default' => happyforms_get_max_upload_size(),
	...
	public function validate_value( $value, $part = array(), $form = array() ) {
		...
		if ( 1 === $part['required'] ) {
			if ( empty( $file_ids ) ) {
				return new WP_Error( 'error', happyforms_get_validation_message( 'file_not_uploaded' ) );
			}
		}
		...
		if ( ! empty( $file_ids[0] ) && count( $attachments ) !== count( $file_ids ) ) {
			return new WP_Error( 'error', happyforms_get_validation_message( 'file_invalid' ) );
		}

		// Check count limit
		if ( ! empty( $attachments ) && ( $part['min_file_count'] > 0 ) && ( $part['min_file_count'] > count( $attachments ) ) ) {
			return new WP_Error( 'error',  happyforms_get_validation_message( 'file_min_count' ) );
		}

		if ( ! empty( $attachments ) && ( $part['max_file_count'] > 0 ) && ( count( $attachments ) > $part['max_file_count'] ) ) {
			return new WP_Error( 'error', __( 'Too many files have been uploaded.', 'happyforms' ) );
		}
	...
	public function email_part_value( $value, $message, $part, $form, $context ) {
		if ( $this->type === $part['type'] ) {
			$parts = $message['parts'];
			$part_id = $part['id'];
			$value = str_replace( ', ', '<br>', happyforms_get_message_part_value( $parts[$part_id], $part, '' ) );
		}
	...
	public function script_dependencies( $deps, $forms ) {
		...
		wp_register_script(
			'happyforms-plupload',
			happyforms_get_plugin_url() . 'inc/assets/js/lib/plupload.min.js',
		...
		wp_register_script(
			...
			array( 'happyforms-plupload' ), happyforms_get_version(), true
		);
	...
	public function style_dependencies( $deps, $forms ) {
		...
		wp_register_style(
			'happyforms-attachment',
			happyforms_get_plugin_url() . 'inc/assets/css/frontend/attachment.css',
	...
	public function transition_allowed_file_extensions( $form ) {
		...
		foreach ( $form['parts'] as $index => $part ) {
			...
			if ( 'deprecated' !== $part['allowed_file_types'] ) {
				$allowed_file_extensions = happyforms_allowed_file_extensions();
				$mime_group = happyforms_get_deprecated_mime_groups();
				$file_extensions = [];
